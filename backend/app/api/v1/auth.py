...
from app.schemas import (
    Token, TokenPair, LoginPayload, UserCreate, UserOut, UserSummaryOut, UserUpdate,
...
def _login_response(db: Session, user: User, request: Request, password_used: str | None = None):
    ...
    if password_used is not None and verify_password("admin123", user.hashed_password):
        must_change = True
    log_audit(db, action="LOGIN_OK", actor=user.username, ip=_client_ip(request),
...
def login(form_data: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db), request: Request = None):
    ...
    if not user:
        ...
        if db_user:
            locked, remain = is_user_locked(db_user, db=db)
            if locked:
                log_audit(db, action="LOGIN_LOCKED", actor=form_data.username, ip=_client_ip(request),
                          detail=f"remain_min={remain}")
                raise HTTPException(status_code=423, detail=f"账户已被锁定，请 {remain} 分钟后再试或联系管理员解锁")
        log_audit(db, action="LOGIN_FAIL", actor=form_data.username, ip=_client_ip(request))
...
def login_json(payload: LoginPayload, db: Session = Depends(get_db), request: Request = None):
    ...
    if not user:
        ...
        if db_user:
            locked, remain = is_user_locked(db_user, db=db)
            if locked:
                log_audit(db, action="LOGIN_LOCKED", actor=payload.username, ip=_client_ip(request),
                          detail=f"remain_min={remain}")
                raise HTTPException(status_code=423, detail=f"账户已被锁定，请 {remain} 分钟后再试或联系管理员解锁")
        log_audit(db, action="LOGIN_FAIL", actor=payload.username, ip=_client_ip(request))
        raise HTTPException(status_code=401, detail="用户名或密码错误")
...
def refresh_token(payload: RefreshTokenIn, db: Session = Depends(get_db), request: Request = None):
    ...
    locked, remain = is_user_locked(user, db=db)
    if locked:
        raise HTTPException(status_code=423, detail=f"账户已被锁定，请 {remain} 分钟后再试或联系管理员解锁")
    access, refresh = create_token_pair(user.username)
    return TokenPair(
...
def change_password(payload: ChangePasswordIn, db: Session = Depends(get_db), current_user=Depends(get_current_user), request: Request = None):
    ...
    log_audit(db, action="PASSWORD_CHANGED", actor=current_user.username, ip=_client_ip(request))
...
@router.post("/logout")
def logout(request: Request = None, current_user=Depends(get_current_user), db: Session = Depends(get_db)):
    """服务端目前不维护 token 黑名单（JWT 是无状态的）；
    客户端应在调用此接口后删除本地 token。
    记录审计日志。
    """
    log_audit(db, action="LOGOUT", actor=current_user.username, ip=_client_ip(request))
    return {"ok": True}
...
def create_user(obj_in: UserCreate, db: Session = Depends(get_db), current_user=Depends(get_current_user), request: Request = None):
    ...
    log_audit(db, action="USER_CREATE", actor=current_user.username, target=u.username,
              ip=_client_ip(request), detail=f"role={u.role}")
    return u


@router.get("/users", response_model=list[UserSummaryOut], dependencies=[Depends(require_roles(UserRole.ADMIN, UserRole.ENGINEER))])
...
def update_user(user_id: int, obj_in: UserUpdate, db: Session = Depends(get_db), current_user=Depends(get_current_user), request: Request = None):
    ...
    log_audit(db, action="USER_UPDATE", actor=current_user.username, target=u.username,
...
def delete_user(user_id: int, db: Session = Depends(get_db), current_user=Depends(get_current_user), request: Request = None):
    ...
    db.delete(db_obj)
    db.commit()
    log_audit(db, action="USER_DELETE", actor=current_user.username, target=target_name,
              ip=_client_ip(request))
    return {"ok": True}
...
def reset_password(user_id: int, payload: ResetPasswordIn, db: Session = Depends(get_db), current_user=Depends(get_current_user), request: Request = None):
    ...
    db.commit()
    log_audit(db, action="PASSWORD_RESET", actor=current_user.username, target=db_obj.username,
              ip=_client_ip(request))
    return {"ok": True}
...
def unlock_user(user_id: int, db: Session = Depends(get_db), current_user=Depends(get_current_user), request: Request = None):
    ...
    db.commit()
    db.refresh(db_obj)
    log_audit(db, action="USER_UNLOCK", actor=current_user.username, target=db_obj.username,
              ip=_client_ip(request))
    return db_obj
