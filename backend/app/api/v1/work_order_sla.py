...
def sla_stats(
    ...
    """SLA 达成率统计：总数/违约数/达成数/达成率/平均响应/平均解决。

    统计范围：已设置 SLA 目标（sla_response_minutes 非空）的工单。
    统计前在内存中重算各工单 SLA（不落库），保证数据新鲜。
    """
    _require(db, current_user.role, "work_order.sla_manage")
    rows = (
        db.query(WorkOrder)
...
@router.get("/breached", response_model=List[WorkOrderOut])
def list_breached(
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
...
@router.post("/{order_id}/set-sla", response_model=WorkOrderOut)
def set_sla(
    order_id: int,
    ...
    _require(db, current_user.role, "work_order.assign")
...
def escalate(
    ...
    if not wo:
        raise HTTPException(status_code=404, detail="工单不存在")
    target = db.query(User).filter(User.id == obj_in.escalated_to_id).first()
    if not target:
        raise HTTPException(status_code=404, detail="升级目标用户不存在")
    wo.escalated = True
    wo.escalated_to_id = obj_in.escalated_to_id
    wo.escalated_at = datetime.utcnow()
    if obj_in.reassign:
        wo.assignee_id = obj_in.escalated_to_id
    _recalc_sla(wo)
    db.commit()
    db.refresh(wo)
    return wo
...
@router.post("/{order_id}/recalculate", response_model=WorkOrderOut)
def recalculate(
    order_id: int,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
